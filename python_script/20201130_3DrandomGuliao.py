...
while True:
    ...
    elif interCheck([disX,disY,disZ],center10,5,5):
        center10.append([disX,disY,disZ])
    count += 1
    if count >= 100:
        break
...
while True:
    ...
    elif interCheck([disX, disY, disZ], center4, 2, 2) and interCheck(
        [disX, disY, disZ], center10, 2, 5
    ):
        center4.append([disX,disY,disZ])
    count += 1
    if count >= 200:
...
while True:
    ...
    if not center1:
        ...
        interCheck([disX, disY, disZ], center1, 0.5, 0.5)
        and interCheck([disX, disY, disZ], center4, 0.5, 2)
        and interCheck([disX, disY, disZ], center10, 0.5, 5)
    ):
        center1.append([disX,disY,disZ])
    count += 1
    if count >= 400:
...
for index in range(len(center10)):
    myAssembly.Instance(
        name=f'Part-Ball-10-{index}',
        part=myModel.parts["Part-Ball-10"],
        dependent=ON,
    )

    myAssembly.translate(
        instanceList=(f'Part-Ball-10-{index}',), vector=tuple(center10[index])
    )

for index in range(len(center4)):
    myAssembly.Instance(
        name=f'Part-Ball-4-{index}',
        part=myModel.parts["Part-Ball-4"],
        dependent=ON,
    )

    myAssembly.translate(
        instanceList=(f'Part-Ball-4-{index}',), vector=tuple(center4[index])
    )

for index in range(len(center1)):
    myAssembly.Instance(
        name=f'Part-Ball-1-{index}',
        part=myModel.parts["Part-Ball-1"],
        dependent=ON,
    )

    myAssembly.translate(
        instanceList=(f'Part-Ball-1-{index}',), vector=tuple(center1[index])
    )
